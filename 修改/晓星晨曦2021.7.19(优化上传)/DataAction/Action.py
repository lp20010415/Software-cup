    ...
    if request.method == 'POST':
        ...
        for f in getFiles:
            ...
            if f.name.split('.')[-1] == 'csv':  # 上传csv文件
                ...
                file.close()
                #   正式上传数据
                os.system(f'''PGPASSWORD=LINUX@123456 psql -h 1.117.74.238 -p 5432 -d match_data -U postgres -c "\copy uploaddata from '{fileURL}' WITH csv header DELIMITER ',' encoding 'utf-8'"''')
                ...
                for i in range(sh.nrows):
                    if i > 0:
                        ...
                        repairTime = datetime.datetime(*xldate_as_tuple(data[0], 0))
                        data[0] = repairTime.strftime('%Y/%m/%d')
                        with open('put.csv', 'a', encoding='utf-8', newline='') as ff:
                            write = csv.writer(ff)
                            write.writerow(data)
                DeleteFile = os.path.join(BASE_DIR, 'put.csv')
                #   正式上传数据
                os.system(f'''PGPASSWORD=LINUX@123456 psql -h 1.117.74.238 -p 5432 -d match_data -U postgres -c "\copy uploaddata from '{DeleteFile}' WITH csv header DELIMITER ',' encoding 'utf-8'"''')
...
def drawTable(request):
    ...
    if request.method == "POST":
        ...
        if order == "desc" or order == "asc":
            cursor.execute(
                f'insert into temporary_table select * from milion_data limit 100 offset {(int(page) - 1) * 100}')
            ...
            if re.match('desc|asc', order.split('-')[-1]):
                ...
                cursor.execute(
                    f"insert into temporary_search_table select * from temporary_table where {field} like '%{content}%'")
    ...
    if request.method == "POST":
        ...
        if len(Xdata) == 10:
            for i, value in enumerate(Xdata):
                cursor.execute(f"select * from sample_1k_flts where {Xvalue} = '{value}' limit 6 offset {num * 3}")
                row = cursor.fetchall()
            ...
            for i, value in enumerate(Xdata):
                cursor.execute(f"select * from sample_1k_flts where {Xvalue} = '{value}' limit 3 offset {num * 3}")
                row = cursor.fetchall()
                for j in range(len(row)):
                    Getdata.append(list(row[j]))
            for i, v1 in enumerate(Getdata):
                for j, v2 in enumerate(v1):
                    if v2 is None:
                        Getdata[i][j] = "0"
            for i in range(0, len(Getdata), 3):
                a = []
                a.append(Getdata[i])
                a.append(Getdata[i + 1])
                a.append(Getdata[i + 2])
