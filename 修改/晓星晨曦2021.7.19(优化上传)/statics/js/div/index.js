let xmSelect = layui.xmSelect                                       //使用xm-select
let GetX_Data, GetX_ChooseItem, GetX_ParentValue, GetX_ParentName   //获取X轴数据，获取X轴选择项，获取X轴父级Value值，获取X轴父级Name
let GetY_ChooseItem = ""                                            //获取Y轴选择项
let GetResponseData = ""                                            //获取后端返回的数据
let YItemName = ['航段距离', '航节主舱位运力量', '旅客量', '主舱位是F和C的旅客量', '团队旅客量', '常旅客量', '每旅客分摊收入', 'Y舱基准价', '座公里', '客公里', '客座率']
let checkEmpty = true
...
let X_Location = xmSelect.render({
    el: '#X_Location',
    height: '200px',
    filterable: true,
    iconfont: {
    ...
    on(data) {
        ...
        if (isAdd && arr.length !== 0) {
            let currData = change.slice(0, 1)
            let getParentValue, getParentName
            try {
...
let Y_Location = xmSelect.render({
    el: '#Y_Location',
    filterable: true,
    style: {
        "width": "250px",
    ...
    data: [
        ...
        {name: 'Y舱基准价', value: 7},
        {name: '座公里', value: 8},    //航节主舱位运力量 * 航段距离 let seat_kil = GetResponseData[]['data'][18-23] * GetResponseData[]['data'][17]
        {name: '客公里', value: 9},   //旅客量 * 航段距离 let guess_kil = GetResponseData[]['data'][20] * GetResponseData[]['data'][17]
        {name: '客座率', value: 10},   //(客公里 / 座公里) let plf = seat_kil / guess_kil
    ],
    on: function (data) {
        let arr = data.arr
        let isAdd = data.isAdd
...
let checkDrawOne = true
let checkDrawTwo = true
let Ydatas = []
let datas = []
let XName = []
let value
let values=[]
let valueArray = [17, 19, 20, 21, 22, 23, 24, 25, 9, 10, 11]
...
//画图表-获取数据
let allGetNum = 0 //计点击确定次数
let PostData = []
function drawCharts_GetData() {
    let GetX_Length = X_Location.getValue().length
    let GetX_Data = X_Location.getValue()
    PostData = []
...
function drawCharts_createElement(length) {
    for (let i = 0; i < length; i++) {
        ...
        if (checkElement){
            ...
            if (value !== 10)
                label.innerHTML = N_DText(true, i)
            else
                label.innerHTML = N_DText(false, i)
        } else{
...
function drawCharts_drawN_D(i, data) {
    let N_DCharts = echarts.init(document.getElementById('N_D' + i))
    let calculate = [0, 0, 0, 0, 0, 0, 0, 0, 0]
    ...
    let option = {
        ...
        tooltip: {
            formatter: function (params) {
                if (YItemName[i] !== "客座率")
                    return "<span style='display:inline-block;margin-right:4px;border-radius:10px;width:10px;height:10px;background-color:#5470c6;'></span>&emsp;" + params[0].name
                else
                    return "<span style='display:inline-block;margin-right:4px;border-radius:10px;width:10px;height:10px;background-color:#5470c6;'></span>&emsp;" + params[0].name + "%"
            },
            trigger: 'axis',
...
//正态分布文本-简化代码
function N_DText(check, i) {
    if (check)
        return "总和:" + sum(datas[i]).toFixed(2) + "&emsp;&emsp;&emsp;平均值:" + average(datas[i]).toFixed(2) + "<br>最大值:" + (Math.max.apply(null, datas[i])).toFixed(2) + "&emsp;&emsp;&emsp;最小值:" + (Math.min.apply(null, datas[i])).toFixed(2) + "<br>方差:" + varianceArr(datas[i]) + "<br>标准差:" + stdDeviation(datas[i])
    else
        return "总和:" + sum(datas[i]).toFixed(2) + "%" + "&emsp;&emsp;&emsp;平均值:" + average(datas[i]).toFixed(2) + "%" + "<br>最大值:" + (Math.max.apply(null, datas[i])).toFixed(2) + "%" + "&emsp;&emsp;&emsp;最小值:" + (Math.min.apply(null, datas[i])).toFixed(2) + "%" + "<br>方差:" + varianceArr(datas[i]) + "%" + "<br>标准差:" + stdDeviation(datas[i]) + "%"
}
