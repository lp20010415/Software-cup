    ...
    if (ord.length !== 0) {
        layer.open({
            ...
            btn: ['确定', '取消'],
            yes: function (index) {
                ScatterDrawCharts_createElement()
                for (let i = 0; i < arrayObj.length; i++) {
                    createrScatterCharts(i)
    ...
    function createrScatterCharts(i) {
        let formatterY = '{value}'
        let ScatterCharts = echarts.init(document.getElementById('charts' + i), 'westeros')
        let scatterSeries = []
        let rotate = 30
        if (xord.length === 10) {
    ...
    function ScatterDrawCharts_createElement() {
        for (let i = 0; i < ord.length; i++) {
